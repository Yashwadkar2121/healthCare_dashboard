...
const Sidebar = ({ onSelectPatient, selectedPatientName }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, x: 0 }}
      transition={{ duration: 0.3 }}
      className="bg-white rounded-2xl shadow-sm p-4 sm:p-5 h-full overflow-y-auto"
    >
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <h3 className="text-xl sm:text-2xl font-bold text-[#072635]">
          Patients
      ...
      <div className="space-y-2 sm:space-y-3 max-h-[calc(100vh-200px)] overflow-y-auto custom-scroll">
        {loading ? (
          <div className="text-center py-8">
            ...
            <motion.div
              ...
              className={`flex items-center justify-between p-2 sm:p-3 rounded-xl cursor-pointer transition-all duration-200 ${
                selectedPatientName === patient.name
                  ? "bg-[#01F0D0]/10 border-2 border-[#01F0D0]"
                  : "hover:bg-gray-50"
              }`}
            >
